...
class TestDistanceArrayDCD(TestCase):
    ...
    def test_Issue4(self):
        ...
        assert_equal(d_ok.shape, (len(self.ca), len(self.ca)), 
                     "copy=True square matrix with N_CA entries")
        assert_equal(d_wrong.shape, (len(self.ca), len(self.ca)), 
                     "copy=False square matrix with N_CA entries")
        # should give the same answer as copy=True but does not, see Issue 4
        assert_almost_equal(d_ok.min(), d_wrong.min(), self.prec,
                            err_msg="copy=False distances != copy=True distances (tested min), known bug")
        assert_almost_equal(d_ok.max(), d_wrong.max(), self.prec,
