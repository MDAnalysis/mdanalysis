...
class Test_PDBDownloaderBaseFunctionality:
    ...
    def test_file_formats(self):
        ...
        for file_format in self.valid_file_formats:
            downloader = mda.web.PDBDownloader(
                PDB_ID=working_PDB_ID, file_format=file_format
            ).download()
            universe_list.append(downloader.convert_to_universe())

        assert all(
            isinstance(universe, mda.Universe) for universe in universe_list
        )
    ...
    def test_timeout(self):
        ...
        with pytest.raises(requests.exceptions.ConnectTimeout):
            mda.web.PDBDownloader(PDB_ID=working_PDB_ID).download(
                timeout=0.0000001
            )
...
class Test_PDBDownloader_Cache:
    ...
    def test_loading_cache(self, shared_cache_directory):
        ...
        downloader1 = mda.web.PDBDownloader(
            PDB_ID=working_PDB_ID, file_format=self.file_format
        )

        downloader1.download(cache_path=shared_cache_directory)
        f1 = shared_cache_directory / f"{working_PDB_ID}.{self.file_format}"

        downloader2 = mda.web.PDBDownloader(
            PDB_ID=working_PDB_ID, file_format=self.file_format
        )
