...
class TestImport:
    ...
    def _create_mock_imdclient(self, version="0.2.2"):
        """Create a complete mock imdclient module with specified version."""
        # Module names
        module_name = "imdclient"
        client_submodule_name = "IMDClient"
        utils_submodule_name = "utils"

        # Mock IMDClient class
        class MockIMDClient:
            pass

        # Main imdclient module
        mock_module = ModuleType(module_name)
        mock_module.__version__ = version

        # imdclient.IMDClient submodule
        mock_client = ModuleType(f"{module_name}.{client_submodule_name}")
        mock_client.IMDClient = MockIMDClient
        mock_module.IMDClient = mock_client

        # imdclient.utils submodule
        mock_utils = ModuleType(f"{module_name}.{utils_submodule_name}")
        mock_utils.parse_host_port = lambda x: ("localhost", 12345)
        mock_module.utils = mock_utils

        return mock_module, mock_client, mock_utils

    def test_has_minversion(self, monkeypatch):
        """Test that HAS_IMDCLIENT is True when imdclient >= MIN_IMDCLIENT_VERSION."""
        # Clean slate for fresh import
        monkeypatch.delitem(
            sys.modules, "MDAnalysis.coordinates.IMD", raising=False
        )

        # Set up mock with compatible version
        mock_module, mock_client, mock_utils = self._create_mock_imdclient(
            str(MIN_IMDCLIENT_VERSION)
        )
        monkeypatch.setitem(sys.modules, "imdclient", mock_module)
        monkeypatch.setitem(sys.modules, "imdclient.IMDClient", mock_client)
        monkeypatch.setitem(sys.modules, "imdclient.utils", mock_utils)

        # Import should succeed with HAS_IMDCLIENT=True
    ...
    def test_no_minversion(self, monkeypatch):
        ...
        # Set up mock with incompatible version
        mock_module, mock_client, mock_utils = self._create_mock_imdclient(
            "0.0.0"
        )
        monkeypatch.setitem(sys.modules, "imdclient", mock_module)
        ...
        # Import should result in HAS_IMDCLIENT=False
        import MDAnalysis.coordinates.IMD
        from MDAnalysis.coordinates.IMD import HAS_IMDCLIENT
    ...
    def test_missing_ImportError(self, monkeypatch):
        ...
        # Set up mock with incompatible version (will set HAS_IMDCLIENT=False)
        mock_main, mock_client, mock_utils = self._create_mock_imdclient(
            "0.0.0"
        )
        monkeypatch.setitem(sys.modules, "imdclient", mock_main)
        monkeypatch.setitem(sys.modules, "imdclient.IMDClient", mock_client)
        monkeypatch.setitem(sys.modules, "imdclient.utils", mock_utils)

        # Import with HAS_IMDCLIENT=False
        import MDAnalysis.coordinates.IMD
        from MDAnalysis.coordinates.IMD import IMDReader

        # IMDReader should raise ImportError
        with pytest.raises(
