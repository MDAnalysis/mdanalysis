...
class _SingleFrameReader(TestCase, RefAdKSmall):
    ...
    def test_n_atoms(self):
        assert_equal(
            self.universe.trajectory.n_atoms,
            self.ref_n_atoms,
...
class BaseReaderTest(object):
    ...
    def test_add_transformations_rejects_non_callable(self):
        ...
        with pytest.raises(TypeError, match="All transformations must be callable"):
    ...
    def test_transformations_copy(self, ref, transformed):
        ...
        for i, ts in enumerate(new):
            ...
            assert_allclose(ts.positions, ideal_coords, atol=10 ** (-ref.prec), rtol=0)
...
def assert_timestep_almost_equal(A, B, decimal=6, verbose=True):
    ...
    if A.frame != B.frame:
        raise AssertionError(
            "A and B refer to different frames: "
            "A.frame = {}, B.frame={}".format(A.frame, B.frame)
        )

    if not np.allclose(A.time, B.time):
        raise AssertionError(
            "A and B refer to different times: "
            "A.time = {}, B.time={}".format(A.time, B.time)
        )
