...
class TestBAT(object):
    ...
    @pytest.fixture()
    def bat(self, selected_residues, scheduler):
        R = BAT(selected_residues)
        R.run(scheduler=scheduler)
        return R.results.bat

    @pytest.fixture
    def bat_npz(self, tmpdir, selected_residues, scheduler):
    ...
    def test_bat_coordinates_single_frame(self, selected_residues, scheduler):
        bat = BAT(selected_residues).run(start=1, stop=2, scheduler=scheduler).results.bat
