...
class TestAtomicDistances(object):
    ...
    def test_ad_pairwise_dist(self, ad_ag1, ad_ag2, expected_dist, client_AtomicDistances):
        """Ensure that pairwise distances between atoms are
        correctly calculated without PBCs."""
        pairwise_no_pbc = ad.AtomicDistances(ad_ag1, ad_ag2, pbc=False).run(**client_AtomicDistances)
    ...
    def test_ad_pairwise_dist_pbc(self, ad_ag1, ad_ag2, expected_pbc_dist, client_AtomicDistances):
        """Ensure that pairwise distances between atoms are
