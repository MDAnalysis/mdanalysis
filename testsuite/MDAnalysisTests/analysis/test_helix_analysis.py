...
class TestHELANAL(object):
    ...
    def test_regression_values(self, scheduler_only_current_process):
        u = mda.Universe(PDB_small)
        ha = hel.HELANAL(u, select='name CA and resnum 161-187',
                         flatten_single_helix=True)
        ha.run(**scheduler_only_current_process)
    ...
    def test_multiple_selections(self, psf_ca, scheduler_only_current_process):
    ...
    def test_residue_gaps_split(self, psf_ca, scheduler_only_current_process):
        sel = 'resid 6:50 or resid 100:130 or resid 132:148'
        with pytest.warns(UserWarning) as rec:
            ha = hel.HELANAL(psf_ca, select=sel).run(**scheduler_only_current_process)
    ...
    def test_residue_gaps_no_split(self, psf_ca, scheduler_only_current_process):
        sel = 'resid 6:50 or resid 100:130 or resid 132:148'
        with pytest.warns(UserWarning) as rec:
    ...
    def test_helanal_zigzag(self, zigzag, ref_axis, screw_angles, scheduler_only_current_process):
        ha = hel.HELANAL(zigzag, select="all", ref_axis=ref_axis,
                         flatten_single_helix=True).run(**scheduler_only_current_process)
