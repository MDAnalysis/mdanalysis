...
class TestDensityAnalysis(DensityParameters):
    def check_DensityAnalysis(self, ag, ref_meandensity,
                              tmpdir, runargs=None, scheduler=None, n_workers=None, **kwargs):
        runargs = runargs if runargs else {}
        with tmpdir.as_cwd():
            D = density.DensityAnalysis(
                ag, delta=self.delta, **kwargs).run(scheduler=scheduler, n_workers=n_workers, **runargs)
    ...
    def test_run(self, mode, universe, tmpdir, schedulers_all):
    ...
    def test_sliced(self, universe, tmpdir, schedulers_all):
        self.check_DensityAnalysis(
    ...
    def test_userdefn_eqbox(self, universe, tmpdir, schedulers_all):
    ...
    def test_userdefn_neqbox(self, universe, tmpdir, schedulers_all):
        self.check_DensityAnalysis(
            universe.select_atoms(self.selections['static']),
    ...
    def test_userdefn_boxshape(self, universe, schedulers_all):
        D = density.DensityAnalysis(
            universe.select_atoms(self.selections['static']),
            delta=1.0, xdim=8.0, ydim=12.0, zdim=17.0,
            gridcenter=self.gridcenters['static_defined']).run(**schedulers_all)
        assert D.results.density.grid.shape == (8, 12, 17)

    def test_warn_userdefn_padding(self, universe, schedulers_all):
        ...
        with pytest.warns(UserWarning, match=regex):
            D = density.DensityAnalysis(
                ...
                gridcenter=self.gridcenters['static_defined']).run(step=5, **schedulers_all)

    def test_warn_userdefn_smallgrid(self, universe, schedulers_all):
    ...
    def test_ValueError_userdefn_gridcenter_shape(self, universe, schedulers_all):
        ...
        with pytest.raises(ValueError, match="Gridcenter must be a 3D coordinate"):
            D = density.DensityAnalysis(
                ...
                gridcenter=self.gridcenters['error1']).run(step=5, **schedulers_all)

    def test_ValueError_userdefn_gridcenter_type(self, universe, schedulers_all):
        # Test gridcenter includes non-numeric strings
        with pytest.raises(ValueError, match="Gridcenter must be a 3D coordinate"):
            D = density.DensityAnalysis(
                universe.select_atoms(self.selections['static']),
                delta=self.delta, xdim=10.0, ydim=10.0, zdim=10.0,
                gridcenter=self.gridcenters['error2']).run(step=5, **schedulers_all)

    def test_ValueError_userdefn_gridcenter_missing(self, universe, schedulers_all):
        # Test no gridcenter provided when grid dimensions are given
        regex = ("Gridcenter or grid dimensions are not provided")
        with pytest.raises(ValueError, match=regex):
            D = density.DensityAnalysis(
                universe.select_atoms(self.selections['static']),
                delta=self.delta, xdim=10.0, ydim=10.0, zdim=10.0).run(step=5, **schedulers_all)

    def test_ValueError_userdefn_xdim_type(self, universe, schedulers_all):
    ...
    def test_ValueError_userdefn_xdim_nanvalue(self, universe, schedulers_all):
    ...
    def test_warn_noatomgroup(self, universe, schedulers_all):
    ...
    def test_ValueError_noatomgroup(self, universe, schedulers_all):
        with pytest.raises(ValueError, match="No atoms in AtomGroup at input"
            ...
            D = density.DensityAnalysis(
                universe.select_atoms(self.selections['none'])).run(step=5, **schedulers_all)

    def test_warn_results_deprecated(self, universe, schedulers_all):
        D = density.DensityAnalysis(
            universe.select_atoms(self.selections['static']))
        D.run(stop=1, **schedulers_all)
