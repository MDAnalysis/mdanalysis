...
def test_notidynamics(u, SELECTION, scheduler):
    with pytest.raises(ImportError, match="tidynamics was not found"):
        u = mda.Universe(PSF, DCD)
        msd = MSD(u, SELECTION)
        msd.run(scheduler=scheduler)
...
class TestMSDSimple(object):
    ...
    def test_simple_step_traj_all_dims(self, step_traj, NSTEP, dim,
        ...
        m_simple.run(scheduler=scheduler)
...
class TestMSDFFT(object):
    ...
    def test_fft_vs_simple_all_dims(self, u, SELECTION, dim, scheduler):
        ...
        m_fft.run(scheduler=scheduler)
        timeseries_fft = m_fft.results.timeseries
        assert_almost_equal(timeseries_simple, timeseries_fft, decimal=4)

    @pytest.mark.parametrize("dim", ['xyz', 'xy', 'xz', 'yz', 'x', 'y', 'z'])
    def test_fft_vs_simple_all_dims_per_particle(self, u, SELECTION, dim, scheduler):
    ...
    def test_fft_step_traj_all_dims(self, step_traj, NSTEP, dim, dim_factor, scheduler):
    ...
    def test_random_walk_u_fft(self, random_walk_u, scheduler):
        # regress against random_walk test data
