...
def calc_bond_distance_ortho(
    ...
    results = distopia.calc_bonds_ortho_float(
        coords1, coords2, box[:3], results=results.astype(np.float32)
    )
...
def calc_bond_distance(
    ...
    results = distopia.calc_bonds_no_box_float(
        coords1, coords2, results=results.astype(np.float32)
    )
    # upcast is currently required, change for 3.0, see #3927
    results = results.astype(np.float64)
