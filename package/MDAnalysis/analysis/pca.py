...
class Fasu(object):
    def __init__(self,
        ...
        trajectory format. In fact Fasu stands for "Filtered and sliced universe"
        (filtered according to atom selection and sliced according to start/stop/step values).
    ...
    def __init__(self,
        ...
        for i in range(num_full_blocks):
            for j in range(blocksize):
                mob = self._f.coords(j + frames_so_far,
                                     reference_structure=self.avg) - self.avg
                trj[j, :] = mob.flatten()
            ...
            if self.captured_variance < 1.0:
                ...
                for i in range(num_full_blocks):
                    for j in range(blocksize):
                        mob = self._f.coords(
                            ...
                            reference_structure=self.avg) - self.avg
                        trj[j, :] = mob.flatten()
                    p[frames_so_far:frames_so_far + blocksize] = np.dot(
                        trj, self.evecs[ev].T).T
                    frames_so_far += blocksize
                ...
                for j in range(remainder):
                    mob = self._f.coords(
                        ...
                        reference_structure=self.avg) - self.avg
                    trj[j, :] = mob.flatten()
                p[frames_so_far:frames_so_far + remainder] = np.dot(
                    trj, self.evecs[ev].T).T
                return p
