...
The computation of the MSD in this way can be computationally intensive due to its :math:`N^2` scaling with respect to :math:`\tau_{max}`. 
...
class EinsteinMSD(AnalysisBase):
    ...
    def __init__(self, u, select=None, msd_type='xyz', fft=True, **kwargs):
        ...
        fft : bool
            If ``True``, uses a fast FFT based algorithm for computation of the MSD.
    ...
    def _parse_msd_type(self):
        ...
        try:
            ...
            raise ValueError(
                'invalid msd_type: {} specified, please specify one of xyz, xy, xz, yz, x, y, z'.format(self.msd_type))
    ...
    def _conclude_simple(self):
        ...
        for lag in lagtimes:
            ...
            sqdist = np.square(disp, dtype=np.float64).sum(
                axis=-1, dtype=np.float64)  # accumulation in np.float64 required
