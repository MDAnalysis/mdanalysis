...
class HydrogenBondAnalysis(AnalysisBase):
    ...
    def __init__(
        ...
        self._donors_sel = (
            donors_sel.strip() if donors_sel is not None else donors_sel
        )
        self._hydrogens_sel = (
            hydrogens_sel.strip()
    ...
    def guess_hydrogens(
        self, select="all", max_mass=1.1, min_charge=0.3, min_mass=0.9
    ):
    ...
    def count_by_time(self):
        ...
        frame_unique, frame_counts = np.unique(
            hbond_frames, return_counts=True
        )
