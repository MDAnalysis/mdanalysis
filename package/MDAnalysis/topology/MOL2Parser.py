...
class MOL2Parser():
    ...
    def parse(self, filename=None):
        ...
        if not len(blocks): raise Exception("The mol2 file '{}' needs to have at least one @<TRIPOS>MOLECULE block".format(filename))
        block = blocks[0]
        ...
        if not len(atom_lines): raise Exception("The mol2 block ({}:{}) has no atoms".format(os.path.basename(filename), block["start_line"]))
        if not len(bond_lines): raise Exception("The mol2 block ({}:{}) has no bonds".format(os.path.basename(filename), block["start_line"]))
