...
class XTCReader:
    """Read a xtc trajectory.

    The xtc format is compressed. As such it does not allow random
    access. Only sequential reading through the trajectory is
    supported.
    """
    def __init__(self,xtcfilename):
        self.source = open(xtcfilename, "rb")
        self.contents = ''
    ...
    def next_frame(self):
        ...
        try:
            ...
            return 0
        if ( flag != 1995): 
            raise RuntimeError("Magic Number Error. Trajectory is unreadable.")
    ...
    def decompress_coordinates(self):
        ...
        if ( (sizeint[0] > 0xffffff) or (sizeint[1] > 0xffffff) or (sizeint[2] > 0xffffff) ):
            bitsizeint = []
            bitsizeint.append(_sizeofint(sizeint[0]))
            bitsizeint.append(_sizeofint(sizeint[1]))
            bitsizeint.append(_sizeofint(sizeint[2]))
            bitsize = 0 #this indicates that we should use the large sizes
        else:
            bitsize = _sizeofints(len(sizeint), sizeint)
        ...
        while(i < self.natoms):
            ...
            if (bitsize == 0):
                for j in range(3):
                    [num,bytes] = _decodebits(bytes, bitsizeint[j])
                    this_coord[j] = (num)
            else:
                [bytes,this_coord] = _decodeints(bytes, 3, bitsize, sizeint)

            i +=1
            for j in range(3):
                this_coord[j] += minint[j]
                prev_coord[j] = this_coord[j]
            [flag,bytes] = _decodebits(bytes, 1)
            is_smaller = 0
            if (flag == 1):
                [run,bytes] = _decodebits(bytes,5)
                is_smaller = run % 3
                run -= is_smaller
                is_smaller -= 1
            if (run > 0):
                for k in range(0,run,3):
                    [bytes,this_coord] = _decodeints(bytes, 3 , smallidx, sizesmall)
                    i += 1
                    for l in range(3):
...
def _sizeofints(length, sizearray):
    num_of_bytes = 1
    num_of_bits = 0
    bytes = []
    bytes.append(1)
...
def _decodebits(stream, num_of_bits):
...
def _decodeints(stream, num_of_ints, num_of_bits, sizes):
    bytes = [0] * 32
    num_of_bytes = 0
    coordinates = [0] * 3
    while(num_of_bits >8):
        [bytes[num_of_bytes],stream] = _decodebits(stream, 8)
        num_of_bytes += 1
        num_of_bits -= 8
    if (num_of_bits > 0):
        [bytes[num_of_bytes],stream] = _decodebits(stream, num_of_bits)
        num_of_bytes += 1
