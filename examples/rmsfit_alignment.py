...
.. Note:: The :func:`rms_fit_trj` function in this example script is
   currently only dealing with dcds, xtcs or pdbs; the extension to
   correctly writing trr files (and testing of xtcs...)  is left as an
   exercise... please submit your code for future inclusion into
   MDAnalysis!
"""
...
import logging
logger = logging.getLogger('MDAnalysis.apps.rmsfit')

class SelectionError(Exception):
    """A error with atom selections occurred."""
...
def rms_fit_trj(traj,ref,select='backbone',filename=None,prefix='rmsfit_'):
    ...
    :Arguments:
      ...
      prefix
         prefix for autogenerating the new dcd filename
      select    
    ...
    instances. If they contain a trajectory then this is used. Alternatively,
    ...
    if isinstance(frames,MDAnalysis.coordinates.XTC.XTCReader):
        ...
        raise TypeError('traj must contain a DCD, XTC, or a PDB.')
...
if __name__ == '__main__':
    ...
    LOGFILE = "mdanalysis.log"

    def setup_logger(logfile=LOGFILE, logger_name='MDAnalysis.apps.rmsfit'):
